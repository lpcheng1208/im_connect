...
type Handler interface {
	...
	OnCheckMessage(c *Conn)          // OnCheckMessage
}
...
type options struct {
	...
	msgResendTime   time.Duration // 不活跃时间
	msgResendTicker time.Duration // 消息重发检测间隔
}
...
// WithMsgResendTime 设置消息重发检查的间隔时间以及超时时间
func WithMsgResendTime(timeoutTicker, timeout time.Duration) Option {
	return newFuncServerOption(func(o *options) {
		if timeoutTicker <= 0 {
			panic("timeoutTicker must greater than 0")
		}
		...
		o.msgResendTicker = timeoutTicker
		o.msgResendTime = timeout
	})
}
...
// Run 启动服务
func (s *Server) Run() {
	logger.Sugar.Info("gn server run")
	s.startAccept()
	s.startIOConsumer()
	s.checkTimeout()
	s.messageResend()
...
func (s *Server) startIOProducer() {
	logger.Sugar.Info("start io producer")
	for {
		select {
		case <-s.stop:
			logger.Sugar.Error("stop producer")
			return
		default:
...
func (s *Server) startAccept() {
	...
	logger.Sugar.Info(fmt.Sprintf("start accept by %d goroutine", s.options.acceptGNum))
}
...
func (s *Server) startIOConsumer() {
	...
	logger.Sugar.Info(fmt.Sprintf("start io event consumer by %d goroutine", len(s.ioEventQueues)))
}
...
func (s *Server) consumeIOEvent(queue chan event) {
	for event := range queue {
		...
		if !ok {
			logger.Sugar.Error("not found in conns,", event.FD)
			continue
		}
		c, ok := v.(*Conn)
		if  !ok {
			logger.Sugar.Error("can not v.(*Conn)")
			continue
		}
...
func (s *Server) checkTimeout() {
	...
	logger.Sugar.Info(fmt.Sprintf("check timeout goroutine run,check_time:%v,timeout:%v", s.options.timeoutTicker, s.options.timeout))
	go func() {
		...
		for {
			...
			case <-s.stop:
				return
			case <-ticker.C:
				logger.Sugar.Debug("now conns: ", s.connsNum)
...
// messageResend 定时检测消息进行重发
func (s *Server) messageResend() {
	logger.Sugar.Info("start loop message resend check ticker")
	go func() {
		ticker := time.NewTicker(s.options.msgResendTicker)
		for {
			select {
			case <-s.stop:
				return
			case <-ticker.C:
				s.conns.Range(func(key, value interface{}) bool {
					c := value.(*Conn)
					if time.Now().Sub(c.lastReadTime) > s.options.msgResendTime {
						return true
					}
					s.handler.OnCheckMessage(c)
					return true
				})
			}
		}
